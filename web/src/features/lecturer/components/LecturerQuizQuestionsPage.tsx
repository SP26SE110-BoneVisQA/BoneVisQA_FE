...
export function LecturerQuizQuestionsPage() {
  ...
  const refresh = (preserveQuestionId?: string) => {
    if (preserveQuestionId && questions.length) {
      const questionIndex = questions.findIndex((q) => q.id === preserveQuestionId);
      if (questionIndex !== -1) {
        setCurrentPage(Math.max(1, Math.ceil((questionIndex + 1) / QUESTIONS_PER_PAGE)));
      }
    }
  ...
  useEffect(() => {
    if (quizQuery.error) {
      setError(quizQuery.error instanceof Error ? quizQuery.error.message : 'Failed to load questions');
    }
  }, [quizQuery.error]);
  ...
  const handleQuestionSuccess = () => {
    ...
    refresh(lastEditedQuestionId ?? undefined);
  };
