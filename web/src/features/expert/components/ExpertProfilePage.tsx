...
import { fetchExpertDashboardStats, type ExpertDashboardStats } from '@/lib/api/expert-dashboard';
...
function StatCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-border bg-card p-5 transition-all hover:shadow-md hover:shadow-warning/5">
...
function RoleBadge({ role }: { role: string }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full border border-warning/20 bg-warning/10 px-2.5 py-0.5 text-[11px] font-semibold text-warning">
...
export function ExpertProfilePage() {
  ...
  const { data: stats } = useQuery({
    queryKey: ['expert-dashboard-stats'],
    queryFn: fetchExpertDashboardStats,
    enabled: clientReady,
    staleTime: 60_000,
  });

  const initials = getInitials(fullName || profile?.fullName || 'E');
  const avatarSrc = pendingAvatarUrl?.trim() || profile?.avatarUrl?.trim() || '';
  const displayName = fullName.trim() || profile?.email?.trim() || 'Expert';
  ...
  return (
    ...
    >
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        {/* Hero Banner */}
        <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-warning/8 via-background to-success/6 shadow-sm">
          <div className="absolute -right-12 -top-12 h-48 w-48 rounded-full bg-warning/5 blur-3xl" />
          <div className="absolute -left-12 -bottom-12 h-40 w-40 rounded-full bg-success/5 blur-3xl" />
          ...
          <div className="relative flex flex-col gap-6 p-8 sm:flex-row sm:items-center sm:gap-8">
            ...
            <div className="flex flex-col items-center sm:items-start">
              ...
              <div className="mt-3 flex flex-wrap justify-center gap-1.5 sm:justify-start">
                {(profile?.roles ?? ['Expert']).map((r) => <RoleBadge key={r} role={r} />)}
              </div>
            </div>
            ...
            <div className="min-w-0 flex-1 text-center sm:text-left">
              ...
              <p className="mt-1.5 flex items-center justify-center gap-1.5 text-sm text-muted-foreground sm:justify-start">
                <Star className="h-3.5 w-3.5 text-warning/70" />
                Medical expert
              </p>
        ...
        {stats ? (
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <StatCard
              ...
              value={fmt(stats.approvedThisMonth)}
            ...
            <StatCard
              ...
              value={fmt(stats.pendingReviews)}
            ...
            <StatCard
              ...
              value={fmt(stats.studentInteractions)}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-6 lg:col-span-2">
            <form id="expert-profile-form" onSubmit={handleSave} className="space-y-6">
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      readOnly={!isEditing}
                      rows={3}
                      placeholder="A short introduction for students and colleagues…"
          ...
          <div className="space-y-6">
            {/* Account Info */}
            <div className="rounded-2xl border border-border bg-gradient-to-br from-warning/4 to-transparent p-6 shadow-sm">
              ...
              <div className="space-y-3 text-xs">
                ...
                {stats && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-foreground">{fmt(stats.totalReviews)}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-dashed border-border p-6">
              ...
              <p className="mb-3 text-xs text-muted-foreground">Manage notifications, review preferences, and more.</p>
              <Button
                type="button"
                variant="outline"
                size="sm"
                className="w-full gap-2"
                onClick={() => { window.location.href = '/expert/settings'; }}
              >
        ...
        <div className="flex flex-wrap items-center justify-end gap-3">
          ...
          <Button type="submit" form="expert-profile-form" isLoading={saving} disabled={saving || uploading}>
