        ...
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="rounded-xl border border-border bg-card p-4 text-center shadow-sm">
              ...
              <p className="text-2xl font-bold text-foreground">{typeof student.quizProgress?.averageScore === 'number' ? `${Number(student.quizProgress.averageScore).toFixed(1)}%` : '—'}</p>
            ...
            <div className="rounded-xl border border-border bg-card p-4 text-center shadow-sm">
              <Clock className={`w-6 h-6 mx-auto mb-2 ${(completionRate ?? 0) >= 70 ? 'text-success' : (completionRate ?? 0) >= 40 ? 'text-warning' : 'text-destructive'}`} />
              <p className="text-2xl font-bold text-foreground">{typeof completionRate === 'number' ? `${Number(completionRate).toFixed(0)}%` : '—'}</p>
          ...
          <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-bold text-card-foreground">{typeof completionRate === 'number' ? `${Number(completionRate).toFixed(0)}%` : '—'}</span>
            </div>
            <div className="h-4 bg-muted rounded-full overflow-hidden">
              <div
                className={`h-full transition-all duration-500 ${getProgressColor(completionRate)}`}
                style={{ width: `${Math.min(100, Math.max(0, completionRate ?? 0))}%` }}
              />
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4 p-4 rounded-lg bg-muted/30">
                  <div className="text-center">
                    ...
                    <p className="text-lg font-bold text-success">{typeof student.quizProgress?.highestScore === 'number' ? `${Number(student.quizProgress.highestScore).toFixed(0)}%` : '—'}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-muted-foreground">Average</p>
                    <p className="text-lg font-bold text-primary">{typeof student.quizProgress?.averageScore === 'number' ? `${Number(student.quizProgress.averageScore).toFixed(1)}%` : '—'}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-muted-foreground">Lowest</p>
                    <p className="text-lg font-bold text-destructive">{typeof student.quizProgress?.lowestScore === 'number' ? `${Number(student.quizProgress.lowestScore).toFixed(0)}%` : '—'}</p>
                  </div>
                </div>
                {/* Quiz List */}
                ...
                <div className="p-4 rounded-lg bg-muted/30">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-lg font-bold ${
                      (student.competencyDetail?.overallCompetency ?? 0) >= 70 ? 'text-success' :
                      (student.competencyDetail?.overallCompetency ?? 0) >= 40 ? 'text-warning' : 'text-destructive'
                    }`}>
                      {typeof student.competencyDetail?.overallCompetency === 'number' ? `${Number(student.competencyDetail.overallCompetency).toFixed(0)}%` : '—'}
                    </span>
                  </div>
                  <div className="h-3 bg-muted rounded-full overflow-hidden">
                    <div
                      className={`h-full ${
                        (student.competencyDetail?.overallCompetency ?? 0) >= 70 ? 'bg-success' :
                        (student.competencyDetail?.overallCompetency ?? 0) >= 40 ? 'bg-warning' : 'bg-destructive'
                      }`}
                      style={{ width: `${Math.min(100, Math.max(0, student.competencyDetail?.overallCompetency ?? 0))}%` }}
                    />
                  </div>
                </div>
                    ...
                    {activity.score != null && (
                      <div className="text-right">
                        <span className={`text-sm font-bold ${
                          (activity.score ?? 0) >= 60 ? 'text-success' : 'text-destructive'
                        }`}>
                          {typeof activity.score === 'number' ? `${Number(activity.score).toFixed(0)}%` : '—'}
                        </span>
                      </div>
                    )}
                  </div>
